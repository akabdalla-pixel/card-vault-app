  ...
  return (
    <nav className="mobile-only" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: 64, background: '#0b0e1c', borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', zIndex: 100 }}>
      {NAV.map(({ label, href }) => {
        const active = typeof window !== 'undefined' && window.location.pathname === href
        const Icon = navIcons[label]
        return <Link key={label} href={href} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, textDecoration: 'none', color: active ? 'var(--cyan)' : '#4a5578', fontFamily: "'Outfit',sans-serif", fontSize: 9, fontWeight: active ? 700 : 500, letterSpacing: '0.04em', textTransform: 'uppercase', paddingBottom: 4 }}><Icon />{label === 'Sold History' ? 'Sold' : label}</Link>
      })}
    </nav>
  )
}
      ...
      <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg)' }}>
        ...
        <main className="main-wrap" style={{ padding: '28px 28px' }}>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <p style={{ fontSize: 13, color: '#4a5578', marginTop: 4, fontWeight: 500 }}>{cards.length} card{cards.length !== 1 ? 's' : ''} sold · complete transaction history</p>
          </div>
          ...
          {cards.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: 12, marginBottom: 22 }}>
              {[
                ...
                ['Realized P&L', (totalPL>=0?'+':'')+fmt(totalPL), totalPL>=0?'#22d3a7':'#ff6b7a'],
                ['Avg Return', (avgReturn>=0?'+':'')+avgReturn.toFixed(1)+'%', avgReturn>=0?'#22d3a7':'#ff6b7a'],
                ['Win Rate', winRate.toFixed(0)+'%', winRate>=50?'#22d3a7':'#ff6b7a'],
              ].map(([label, value, color]) => (
                <div key={label} style={{ padding: '12px 14px', borderRadius: 12, background: 'linear-gradient(135deg,#131929,#0f1521)', border: '1px solid rgba(255,255,255,0.07)' }}>
                  <div style={{ fontSize: 10, color: '#3a4465', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 6 }}>{label}</div>
          ...
          {cards.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '80px 24px' }}>
              ...
              <h3 style={{ fontFamily: "'Outfit',sans-serif", fontSize: 18, fontWeight: 700, color: '#3a4465', marginBottom: 8 }}>No sales yet</h3>
              <p style={{ color: '#2e3759', fontSize: 13, marginBottom: 20, fontFamily: "'Outfit',sans-serif" }}>Mark cards as sold from your Collection to track your realized gains here.</p>
              <Link href="/collection" style={{ padding: '10px 20px', borderRadius: 10, background: 'rgba(6,214,214,0.08)', border: '1px solid rgba(6,214,214,0.25)', color: 'var(--cyan)', fontFamily: "'Outfit',sans-serif", fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>Go to Collection</Link>
            </div>
          ) : (
            <div style={{ background: 'linear-gradient(135deg,#131929,#0f1521)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden' }}>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>{['Card','Sport','Grade','Buy Price','Sold Price','Profit/Loss','Return','Sold Date'].map((h,i) => <th key={h} style={{ padding: '11px 16px', textAlign: i===0?'left':'right', fontFamily: "'Outfit',sans-serif", fontSize: 10, fontWeight: 700, color: '#2e3759', letterSpacing: '0.1em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>)}</tr>
                  </thead>
                  <tbody>
                    {[...cards].sort((a,b) => new Date(b.soldDate||b.updatedAt) - new Date(a.soldDate||a.updatedAt)).map(card => {
                      ...
                      return (
                        <tr key={card.id} className="sold-row" style={{ borderTop: '1px solid rgba(255,255,255,0.04)', transition: 'background 0.15s' }}>
                          <td style={{ padding: '12px 16px' }}>
                            ...
                            {card.year && <div style={{ fontSize: 11, color: '#3a4465' }}>{card.year}</div>}
                          </td>
                          <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: "'Outfit',sans-serif", fontSize: 12, color: '#6a75a0' }}>{card.sport||'—'}</td>
                          <td style={{ padding: '12px 16px', textAlign: 'right' }}>{card.grade ? <span style={{ padding: '2px 8px', borderRadius: 6, background: 'rgba(6,214,214,0.1)', color: 'var(--cyan)', fontSize: 11, fontWeight: 700 }}>PSA {card.grade}</span> : <span style={{ color: '#3a4465', fontSize: 12 }}>{card.cond||'Raw'}</span>}</td>
                          <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: "'JetBrains Mono',monospace", fontSize: 13, color: '#6a75a0' }}>{fmt(buy)}</td>
                          ...
                          <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 3, fontFamily: "'JetBrains Mono',monospace", fontSize: 13, fontWeight: 700, color: plPos?'#22d3a7':'#ff6b7a' }}>{plPos?<IconUp />:<IconDown />}{plPos?'+':''}{fmt(pl)}</div>
                          </td>
                          <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: "'JetBrains Mono',monospace", fontSize: 12, fontWeight: 600, color: plPos?'#22d3a7':'#ff6b7a' }}>{plPos?'+':''}{pct.toFixed(1)}%</td>
                          <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: "'Outfit',sans-serif", fontSize: 12, color: '#4a5578' }}>{card.soldDate ? new Date(card.soldDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}</td>
                        </tr>
                      )
                    })}
                  </tbody>
