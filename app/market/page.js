...
export default function MarketPage() {
  ...
  function openAddModal(listing) {
    ...
    const sportMap = { Basketball:['nba','lebron','jordan','kobe','curry','durant','lakers','celtics'], Football:['nfl','mahomes','brady','manning'], Baseball:['mlb','baseball'], Soccer:['messi','ronaldo','premier league'], F1:['f1','formula','ferrari','verstappen','hamilton'] }
    let sport = ''
  ...
  return (
    <>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:20 }}>
              {[
                ...
                <div key={i} style={{ background:'#111', border:'1px solid #1a1a1a', borderRadius:10, padding:'12px 14px', position:'relative', overflow:'hidden' }}>
                  ...
                  <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:16, fontWeight:900, color:'#fff' }}>{s.value}</div>
                </div>
              ))}
            </div>
          )}
                ...
                <a key={i} href={r.link} target="_blank" rel="noopener noreferrer" className="listing-card" style={{ background:'#111', border:'1px solid #1a1a1a', borderRadius:12, overflow:'hidden', textDecoration:'none', display:'block' }}>
                  ...
                  <div style={{ padding:'10px 12px' }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                      <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:15, fontWeight:900, color:'#fff' }}>{fmt(r.price)}</div>
      ...
      {addModal && <CardModal card={addModal} onClose={() => setAddModal(null)} onSave={() => { setAddModal(null); setAddSuccess(true); setTimeout(() => setAddSuccess(false), 3000) }} />}
    </>
  )
}
