      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-5">
            ...
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} required className="w-full p-3 rounded-xl text-base outline-none" style={{ background: 'var(--card)', border: '1px solid var(--border)', color: 'var(--text)' }} placeholder="Your password" />
          </div>
          <button type="submit" disabled={loading} className="w-full p-3.5 rounded-xl text-base font-bold text-black disabled:opacity-50" style={{ background: 'linear-gradient(135deg, var(--cyan), var(--cyan2))' }}>{loading ? 'Signing in...' : 'Sign In'}</button>
        </form>
        <p className="text-center mt-6 text-sm" style={{ color: 'var(--dim)' }}>No account? <Link href="/signup" className="font-semibold" style={{ color: 'var(--cyan)' }}>Sign Up</Link></p>
      </div>
    </div>
  )
}
