...
function ToastContainer() {
  ...
  return (
    <div style={{ position:'fixed', bottom:88, left:'50%', transform:'translateX(-50%)', zIndex:9999, display:'flex', flexDirection:'column', gap:8, alignItems:'center', pointerEvents:'none' }}>
      {toasts.map(t => (
        <div key={t.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 18px', borderRadius:12, background:'#1e1e1e', border:`1px solid ${colors[t.type]}50`, boxShadow:'0 8px 28px rgba(0,0,0,0.6)', fontFamily:"'Unbounded',sans-serif", fontSize:13, fontWeight:600, color:'#f0f0f0', animation:'toastIn 0.2s ease', whiteSpace:'nowrap' }}>
  ...
  return (
    <nav style={{ position:'fixed', bottom:0, left:0, right:0, height:76, background:'#0d0d0d', borderTop:'1px solid #1e1e1e', display:'flex', alignItems:'center', zIndex:100 }} className="mobile-only">
      {[...NAV, { label:'Settings', href:'/settings' }].slice(0,4).map(({ label, href }) => {
        ...
        return <Link key={label} href={href} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:4, textDecoration:'none', color:isActive?'#9333ea':'#555', fontFamily:"'Unbounded',sans-serif", fontSize:9, fontWeight:isActive?700:500, letterSpacing:'0.04em', textTransform:'uppercase', paddingBottom:4 }}><Icon />{label==='Sold History'?'Sold':label}</Link>
      })}
    </nav>
  )
}
  ...
  return (
    <div style={{ background: danger ? 'linear-gradient(135deg,#1a0808,#0d0d0d)' : 'linear-gradient(135deg,#111,#0d0d0d)', border: `1px solid ${danger ? 'rgba(147,51,234,0.25)' : '#1e1e1e'}`, borderRadius:14, padding:22, position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16 }}>
        ...
        <div>
          <h2 style={{ fontFamily:"'Unbounded',sans-serif", fontSize:14, fontWeight:700, color: danger ? '#a855f7' : '#ccc', margin:0 }}>{title}</h2>
          {subtitle && <p style={{ fontSize:12, color:'#444', margin:'3px 0 0', fontFamily:"'Unbounded',sans-serif" }}>{subtitle}</p>}
        </div>
      </div>
      {children}
    </div>
  ...
  const pwInput = (key, placeholder) => (
    <div style={{ position:'relative' }}>
      <input
        ...
        style={{ width:'100%', padding:'10px 40px 10px 14px', borderRadius:10, background:'#202020', border:'1px solid #2a2a2a', color:'#f0f0f0', fontSize:14, outline:'none', fontFamily:"'Unbounded',sans-serif", boxSizing:'border-box' }}
      />
      ...
      <div style={{ display:'flex', minHeight:'100vh', background:'#0a0a0a' }}>
        ...
        <main className="main-wrap" style={{ padding:'30px 28px' }}>
          ...
          <div className="mob-topbar" style={{ alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
            ...
            <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:16, fontWeight:800, color:'#f0f0f0' }}>Settings</div>
            <div style={{ width:32 }} />
          </div>

          {/* Header */}
          <div style={{ marginBottom:28 }} className="hide-mobile">
            <h1 style={{ fontFamily:"'Unbounded',sans-serif", fontSize:24, fontWeight:800, color:'#f0f0f0', letterSpacing:'-0.5px', margin:0 }}>Settings</h1>
          ...
          <div style={{ maxWidth:560, display:'flex', flexDirection:'column', gap:18 }}>
            ...
            {/* ── Account Info ── */}
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                {[['Username', `@${user?.username}`], ['Email', user?.email]].map(([label, val]) => (
                  <div key={label}>
                    <div style={{ fontSize:10, fontWeight:700, color:'#444', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:4, fontFamily:"'Unbounded',sans-serif" }}>{label}</div>
                    <div style={{ padding:'10px 14px', borderRadius:10, background:'#202020', border:'1px solid #2a2a2a', fontFamily:"'Unbounded',sans-serif", fontSize:14, color:'#888' }}>{val}</div>
                  </div>
                ))}
              </div>
            </Section>
            ...
            <Section title="Change Name" subtitle="Update your display name" icon="✏️">
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                <div style={{ fontSize:12, color:'#555', fontFamily:"'Unbounded',sans-serif", lineHeight:1.5 }}>
                  Current name: <span style={{ color:'#a855f7', fontWeight:700 }}>{user?.username ? user.username[0].toUpperCase() + user.username.slice(1) : ''}</span>
                </div>
                {nameError && <div style={{ padding:'10px 14px', borderRadius:10, background:'rgba(147,51,234,0.08)', border:'1px solid rgba(147,51,234,0.2)', color:'#9333ea', fontSize:13, fontFamily:"'Unbounded',sans-serif" }}>{nameError}</div>}
                <div>
                  <div style={{ fontSize:10, fontWeight:700, color:'#444', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6, fontFamily:"'Unbounded',sans-serif" }}>New Name</div>
                  <input
                    type="text"
                    value={nameVal}
                    onChange={e => setNameVal(e.target.value)}
                    placeholder="Enter new name"
                    style={{ width:'100%', padding:'10px 14px', borderRadius:10, background:'#202020', border:'1px solid #2a2a2a', color:'#f0f0f0', fontSize:14, outline:'none', fontFamily:"'Unbounded',sans-serif", boxSizing:'border-box' }}
                  />
                </div>
                <button type="button" onClick={handleChangeName} disabled={nameLoading || !nameVal.trim()} style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, padding:'11px 20px', borderRadius:10, background:'rgba(147,51,234,0.1)', border:'1px solid rgba(147,51,234,0.3)', color:'#9333ea', fontFamily:"'Unbounded',sans-serif", fontSize:14, fontWeight:700, cursor:'pointer', opacity: !nameVal.trim() ? 0.4 : 1 }}>
            ...
            <Section title="Change Password" subtitle="Update your login password" icon="🔒">
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                {pwError && <div style={{ padding:'10px 14px', borderRadius:10, background:'rgba(147,51,234,0.08)', border:'1px solid rgba(147,51,234,0.2)', color:'#9333ea', fontSize:13, fontFamily:"'Unbounded',sans-serif" }}>{pwError}</div>}
                <div>
                  <div style={{ fontSize:10, fontWeight:700, color:'#444', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6, fontFamily:"'Unbounded',sans-serif" }}>Current Password</div>
                  {pwInput('current', 'Enter current password')}
                </div>
                <div>
                  <div style={{ fontSize:10, fontWeight:700, color:'#444', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6, fontFamily:"'Unbounded',sans-serif" }}>New Password</div>
                  {pwInput('next', 'At least 8 characters')}
                </div>
                <div>
                  <div style={{ fontSize:10, fontWeight:700, color:'#444', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6, fontFamily:"'Unbounded',sans-serif" }}>Confirm New Password</div>
                  {pwInput('confirm', 'Repeat new password')}
                </div>
                <button type="button" disabled={pwLoading || !pwForm.current || !pwForm.next || !pwForm.confirm} style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, padding:'11px 20px', borderRadius:10, background:'rgba(147,51,234,0.1)', border:'1px solid rgba(147,51,234,0.3)', color:'#9333ea', fontFamily:"'Unbounded',sans-serif", fontSize:14, fontWeight:700, cursor:'pointer', opacity: (!pwForm.current||!pwForm.next||!pwForm.confirm) ? 0.4 : 1 }}>
            ...
            <Section title="Export & Backup" subtitle="Download a copy of your collection data" icon="💾">
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                <div style={{ padding:'14px 16px', background:'#1a1a1a', borderRadius:10, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  <div>
                    <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:13, fontWeight:700, color:'#ccc' }}>Export as CSV</div>
                    <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:11, color:'#555', marginTop:2 }}>Compatible with Excel, Google Sheets, and import back into TopLoad</div>
                  </div>
                  <button onClick={exportCSV} disabled={!cards.length} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 16px', borderRadius:10, background:'rgba(147,51,234,0.08)', border:'1px solid rgba(147,51,234,0.2)', color:'#9333ea', fontFamily:"'Unbounded',sans-serif", fontSize:13, fontWeight:600, cursor:'pointer', flexShrink:0, opacity:cards.length?1:0.4 }}>
                    <IconDownload />CSV
                  </button>
                </div>
                <div style={{ padding:'14px 16px', background:'#1a1a1a', borderRadius:10, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  <div>
                    <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:13, fontWeight:700, color:'#ccc' }}>Export as JSON</div>
                    <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:11, color:'#555', marginTop:2 }}>Full data backup including all fields and metadata</div>
                  </div>
                  <button onClick={exportJSON} disabled={!cards.length} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 16px', borderRadius:10, background:'rgba(255,255,255,0.05)', border:'1px solid #2a2a2a', color:'#888', fontFamily:"'Unbounded',sans-serif", fontSize:13, fontWeight:600, cursor:'pointer', flexShrink:0, opacity:cards.length?1:0.4 }}>
                    <IconDownload />JSON
                  </button>
                </div>
                <div style={{ fontSize:11, color:'#444', fontFamily:"'Unbounded',sans-serif", textAlign:'center' }}>
            ...
            <Section title="Danger Zone" subtitle="These actions are permanent and cannot be undone" icon="⚠️" danger>
              ...
              <div style={{ borderRadius:12, border:'1px solid rgba(147,51,234,0.2)', overflow:'hidden', marginBottom:12 }}>
                <div style={{ padding:'14px 16px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
                  <div>
                    <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:13, fontWeight:700, color:'#ccc' }}>Delete All Cards</div>
                    <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:11, color:'#555', marginTop:2 }}>Permanently removes all {cards.length} cards from your collection</div>
                  </div>
                  <button onClick={() => setShowDeleteCards(!showDeleteCards)} disabled={!cards.length} style={{ padding:'8px 14px', borderRadius:10, background:'rgba(147,51,234,0.08)', border:'1px solid rgba(147,51,234,0.25)', color:'#9333ea', fontFamily:"'Unbounded',sans-serif", fontSize:13, fontWeight:600, cursor:'pointer', flexShrink:0, opacity:cards.length?1:0.4 }}>
                ...
                {showDeleteCards && (
                  <div style={{ padding:'14px 16px', borderTop:'1px solid rgba(147,51,234,0.15)', background:'rgba(147,51,234,0.04)' }}>
                    <p style={{ fontFamily:"'Unbounded',sans-serif", fontSize:13, color:'#9333ea', marginBottom:12, display:'flex', alignItems:'center', gap:6 }}>
                    ...
                    <div style={{ display:'flex', gap:10 }}>
                      <input
                        ...
                        style={{ flex:1, padding:'9px 14px', borderRadius:10, background:'#111', border:'1px solid rgba(147,51,234,0.3)', color:'#f0f0f0', fontSize:14, outline:'none', fontFamily:"'Unbounded',sans-serif" }}
                      />
                      <button onClick={handleDeleteAllCards} disabled={deleteCardsConfirm!=='DELETE'||deleteCardsLoading} style={{ padding:'9px 16px', borderRadius:10, background: deleteCardsConfirm==='DELETE' ? 'rgba(147,51,234,0.15)' : 'rgba(255,255,255,0.04)', border:'1px solid rgba(147,51,234,0.3)', color: deleteCardsConfirm==='DELETE' ? '#9333ea' : '#444', fontFamily:"'Unbounded',sans-serif", fontSize:13, fontWeight:700, cursor:'pointer', flexShrink:0 }}>
              ...
              <div style={{ borderRadius:12, border:'1px solid rgba(147,51,234,0.2)', overflow:'hidden' }}>
                <div style={{ padding:'14px 16px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
                  <div>
                    <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:13, fontWeight:700, color:'#9333ea' }}>Delete Account</div>
                    <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:11, color:'#555', marginTop:2 }}>Permanently deletes your account and all data. Cannot be undone.</div>
                  </div>
                  <button onClick={() => setShowDeleteAccount(!showDeleteAccount)} style={{ padding:'8px 14px', borderRadius:10, background:'rgba(147,51,234,0.15)', border:'1px solid rgba(147,51,234,0.4)', color:'#9333ea', fontFamily:"'Unbounded',sans-serif", fontSize:13, fontWeight:700, cursor:'pointer', flexShrink:0 }}>
                  ...
                  <div style={{ padding:'14px 16px', borderTop:'1px solid rgba(147,51,234,0.15)', background:'rgba(147,51,234,0.04)' }}>
                    ...
                    <div style={{ display:'flex', gap:10 }}>
                      <input
                        ...
                        style={{ flex:1, padding:'9px 14px', borderRadius:10, background:'#111', border:'1px solid rgba(147,51,234,0.3)', color:'#f0f0f0', fontSize:14, outline:'none', fontFamily:"'Unbounded',sans-serif" }}
                      />
                      <button onClick={handleDeleteAccount} disabled={deleteAccountConfirm!==user?.username||deleteAccountLoading} style={{ padding:'9px 16px', borderRadius:10, background: deleteAccountConfirm===user?.username ? 'rgba(147,51,234,0.2)' : 'rgba(255,255,255,0.04)', border:'1px solid rgba(147,51,234,0.4)', color: deleteAccountConfirm===user?.username ? '#9333ea' : '#444', fontFamily:"'Unbounded',sans-serif", fontSize:13, fontWeight:700, cursor:'pointer', flexShrink:0 }}>
            ...
            <div style={{ paddingBottom:8 }}>
              <button onClick={handleLogout} style={{ display:'flex', alignItems:'center', gap:8, padding:'11px 20px', borderRadius:10, background:'rgba(255,255,255,0.04)', border:'1px solid #2a2a2a', color:'#555', fontFamily:"'Unbounded',sans-serif", fontSize:14, fontWeight:600, cursor:'pointer' }}>
                <IconLogout />Sign Out
              </button>
            </div>
