        ...
        <div style={{ display:'flex', gap:8 }}>
          {data.map((d,i) => (
            <div key={i} style={{ flex:1, background: i===0?'rgba(147,51,234,0.1)':'rgba(255,255,255,0.04)', border: i===0?'1px solid rgba(147,51,234,0.2)':'1px solid #1e1e1e', borderRadius:9, padding:'10px 12px', textAlign:'center' }}>
              <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:22, fontWeight:900, color: i===0?'#a855f7':'#888' }}>{d.value}</div>
            ...
            return (
              <div key={i}>
                <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                  ...
                  <span style={{ fontSize:11, fontWeight:800, color:'#a855f7', fontFamily:"'Unbounded',sans-serif" }}>{pct}%</span>
                </div>
...
function BarChart({ data, title, valuePrefix = '', valueSuffix = '', color = RED }) {
  ...
  return (
    <div style={{ background: '#111', border: '1px solid #1e1e1e', borderRadius: 14, padding: '16px' }}>
      <div style={{ fontFamily: "'Unbounded',sans-serif", fontSize: 13, fontWeight: 700, color: '#ccc', marginBottom: 18 }}>{title}</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {data.map((d, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}
            onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)}>
            <div style={{ width: 80, fontFamily: "'Unbounded',sans-serif", fontSize: 11, color: '#666', textAlign: 'right', flexShrink: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{d.label}</div>
            ...
            <div style={{ width: 60, fontFamily: "'Unbounded',sans-serif", fontSize: 12, fontWeight: 700, color: hovered === i ? '#f0f0f0' : '#888', textAlign: 'right', flexShrink: 0 }}>{valuePrefix}{d.value}{valueSuffix}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ background: '#111', border: `1px solid ${accent ? 'rgba(147,51,234,0.25)' : '#1e1e1e'}`, borderRadius: 16, padding: '16px 18px', position: 'relative', overflow: 'hidden', ...style }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: '#333', letterSpacing: '0.12em', textTransform: 'uppercase', fontFamily: "'Unbounded',sans-serif" }}>{label}</div>
        {icon && <div style={{ fontSize: 16 }}>{icon}</div>}
      </div>
      <div style={{ fontFamily: "'Unbounded',sans-serif", fontSize: 20, fontWeight: 700, color: '#f0f0f0', letterSpacing: '-0.5px', lineHeight: 1 }}>{value}</div>
      {sub && <div style={{ marginTop: 8, fontSize: 11, color: positive === true ? '#22c55e' : positive === false ? '#ef4444' : '#555', fontFamily: "'Unbounded',sans-serif", fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
...
function TopCardsRank({ cards }) {
  ...
  return (
    <div style={{ background: '#111', border: '1px solid #1e1e1e', borderRadius: 14, padding: '16px' }}>
      <div style={{ fontFamily: "'Unbounded',sans-serif", fontSize: 13, fontWeight: 700, color: '#ccc', marginBottom: 18, display: 'flex', alignItems: 'center', gap: 8 }}><IconTrophy /><span>Top 5 Most Valuable</span></div>
        ...
        {top.map((card, i) => {
          ...
          return (
            <Link key={card.id} href={href} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 10px', borderRadius: 10, textDecoration: 'none', transition: 'background 0.12s' }}
              ...
              <div style={{ width: 24, height: 24, borderRadius: 8, background: i === 0 ? 'rgba(147,51,234,0.2)' : '#1a1a1a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Unbounded',sans-serif", fontSize: 11, fontWeight: 700, color: i === 0 ? '#9333ea' : '#555', flexShrink: 0 }}>{i + 1}</div>
              ...
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                <div style={{ fontFamily: "'Unbounded',sans-serif", fontSize: 14, fontWeight: 700, color: '#f0f0f0' }}>{fmt(val)}</div>
                <div style={{ fontFamily: "'Unbounded',sans-serif", fontSize: 11, color: glPos ? '#22c55e' : '#ef4444', marginTop: 2 }}>{glPos ? '+' : ''}{fmt(gl)}</div>
              </div>
        ...
        {records.map((r, i) => {
          ...
          const inner = (
            <div style={{ background:'#181818', border:'1px solid #1e1e1e', borderRadius:10, padding:'10px 12px', cursor:href?'pointer':'default', transition:'background 0.12s', height:'100%' }}
              ...
              <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:11, fontWeight:800, color:r.subColor }}>{r.sub}</div>
            </div>
          )
          return href
      ...
      <div style={{ display: 'flex', minHeight: '100vh', background: '#0a0a0a' }}>
        <div className="sidebar-el"><Sidebar user={user} onLogout={handleLogout} active="Insights" cardCount={activeCards.length} /></div>          <main className="main-wrap" style={{ padding: '28px 28px 40px' }}>
          ...
          <div className="mob-topbar" style={{ alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            ...
            <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize: 16, fontWeight: 800, color: '#f0f0f0' }}>Insights</div>
            <div style={{ width: 32 }} />
          </div>

          {/* Desktop header */}
          <div className="hide-mobile" style={{ marginBottom: 28 }}>
            <h1 style={{ fontFamily: "'Unbounded',sans-serif", fontSize: 40, fontWeight: 900, letterSpacing: '-2px', margin: 0, background: 'linear-gradient(135deg,#f5f5f5 40%,#a855f7)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Insights</h1>
          ...
          {isEmpty ? (
            <div style={{ textAlign: 'center', padding: '100px 24px' }}>
              ...
              <p style={{ fontFamily: "'Unbounded',sans-serif", fontSize: 15, color: '#333' }}>Add cards to your collection to see insights</p>
            </div>
          ) : (
            <>
              {/* ── Mobile Hero ── */}
